...
When I tried set it up however, I was swarmed with numerous issues found by `dependabot` and the staleness of settings brought about a lot of headaches, especially due to the node version. I mean it was off for 2 years so there's that.

I could've fixed it but I really wanted a place to write without having the issues of configuration at the moment. I thought I about using another `Gatsby.js` starter but due to it being in the process of updating to V4 I was hesitant due to the extra configurations that will eventually come up.
...
It was not without problems however since during the course of the blog's setup I spotted a small issue on its disqus configuration in `Disqus.js`.
...
- Game Development using Unity (since I'm currently building 3D Games on the side)
